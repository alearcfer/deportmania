...
class BaseCart(models.Model):
    ...
    def add_product(self,product,talla, quantity ,merge=True, queryset=None):
        """
        ...
        print(item)
        i=0
        print("item",len(queryset))
        # Let's see if we already have an Item with the same product ID
        #Modificado por ale
        if item.exists() and merge:
            while i< len(item):
                print(len(item))
                print("Comprobacion nº",i)
                cart_item = item[i]
                print(cart_item.talla)
                if cart_item.talla==talla:
                    print("llega al if")
                    cart_item.quantity = cart_item.quantity + int(quantity)
                    cart_item.save()
                    break
                if i == len(item)-1:
                    print("Llegamos al final, por lo que añadimos uno nuevo")
                    cart_item = CartItem.objects.create(
                    cart=self, quantity=quantity, product=product,talla=talla)
                    cart_item.save()
                    break
                else:
                    i= i+1
